...
function OrderForm({ initialType = 'fence', initialId = '' }) {
  ...
  const noInstallTotal = Math.round(totalSqm * sqmPrice);
  const installTotal = Math.round(totalSqm * (sqmPrice + 12000));
  const finalPrice = install === 'yes' ? installTotal : noInstallTotal;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto bg-white rounded-lg shadow p-6 flex flex-col gap-4">
      ...
      {success && <div className="bg-green-100 text-green-900 p-2 rounded">{success}</div>}
      {error && <div className="bg-red-100 text-red-900 p-2 rounded">{error}</div>}
      {/* Product details */}
      {selected && selected._id && (
        <div className="border rounded-lg p-4 mb-2 flex gap-4 bg-gray-50">
          <img src={selected.image || 'https://via.placeholder.com/120x80'} alt={selected.name || selected.type} className="w-32 h-24 object-cover rounded" />
      ...
      {/* Form fields */}
      <div>
        <label className="block mb-1 font-medium">Захиалагчийн нэр</label>
        <input value={name} onChange={e => setName(e.target.value)} required className="w-full border rounded px-3 py-2" />
      </div>
      <div>
        <label className="block mb-1 font-medium">Захиалагчийн утас</label>
      ...
      <div>
        ...
        <select value={itemType} onChange={e => { setItemType(e.target.value); setItemId(''); }} className="w-full border rounded px-3 py-2">
      ...
      {selected && selected._id && (
        <div className="border rounded-lg p-4 mb-2 flex flex-col gap-2 bg-gray-50">
          <div className="text-sm text-gray-700 mb-1">
            ...
            <span>1 м² үнэ: <b>{sqmPrice.toLocaleString()}₮</b></span>
          </div>
      ...
      <button type="submit" disabled={loading} className="bg-green-900 text-white rounded px-4 py-2 font-bold mt-2 hover:bg-green-800 disabled:opacity-50">
